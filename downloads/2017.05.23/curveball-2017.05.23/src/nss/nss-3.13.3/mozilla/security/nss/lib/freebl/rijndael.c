...
#define pOut output
    ...
    if (0) { /* CURVEBALL */
	unsigned int j;
	printf("XX aes128 keyprefix ");

	for (j = 0; j < 12; j++) {
	    printf("%.8x ", cx->expandedKey[j]);
	}
	printf("\n");
    } /* end CURVEBALL */
                    ...
                    unsigned int blocksize)
{
	...
	printf("encryptCBC IV (%d) %d : ", inputLen, blocksize);
	for (j = 0; j < 16; j++) {
	    printf("%.2x", lastblock[j]);
	}
	printf("\n");
    }
    /* end CURVEBALL */
	        ...
	        unsigned int blocksize)
{
	...
	curveball_aes_set_context(cx);
	curveball_aes_set_key(key, keysize);

	if (0) {
	    printf("AES_InitContext!!!\n");
	    printf("-------- AES KEY mode %s size %d: ", encrypt ? "enc" : "dec", keysize);
	    for (j = 0; j < keysize; j++) {
		printf("%.2x", key[j]);
	    }
	    printf("\n");
	...
	if (encrypt) {
	    unsigned int j;
	    printf("XX AES SETUP keyprefix NK %d: ", Nk);
