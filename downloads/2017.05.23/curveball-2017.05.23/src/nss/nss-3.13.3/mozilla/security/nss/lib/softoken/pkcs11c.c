...
jpake1:
	...
	/* CURVEBALL */
	/* If this is a Curveball connection, then create the Premaster
	 * Secret using the Curveball protocol instead of selecting it
	 * entirely at random
	 */
	if (version->major & CURVEBALL_PMS_FLAG) {
	    CURVEBALL_PMS_DATA *curveball_data =
		    (CURVEBALL_PMS_DATA *) pMechanism->pParameter;
	    version = &curveball_data->version;
	    rsa_pms->client_version[0] = version->major & ~CURVEBALL_PMS_FLAG;
	    rsa_pms->client_version[1] = version->minor;

	    memcpy(&rsa_pms->random[0], &curveball_data->random[0],
		    sizeof(rsa_pms->random));
	    crv = CKR_OK;
        }
        else {
