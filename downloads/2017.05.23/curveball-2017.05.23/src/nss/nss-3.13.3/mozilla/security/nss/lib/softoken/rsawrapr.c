...
/* CURVEBALL */
#include "../ssl/curveball_premaster.h"
/* end CURVEBALL */
		   ...
		   SECItem *data)
{
	...
	*bp++ = RSA_BLOCK_AFTER_PAD_OCTET;
	...
	/* CURVEBALL */
#ifdef CURVEBALL_IN_OTHER_LIBRARIES
        /* (Can't include curveball_is_enabled because it's not defined in
         * this shared library)
         */
        if (data->len == CURVEBALL_PMS_TOTAL_BYTES
		/* && curveball_is_enabled(7) */) {
            /* If we might be encrypting the premaster secret, we need
             * something predictable in the padding, not random
             */
            unsigned char *pbp = bp;
            int stop_random = modulusLen - 2;
            
            for (pbp = bp, i = 0;
		    i + data->len < stop_random;
		    pbp += data->len, i+= data->len) {
                PORT_Memcpy(pbp, data->data, data->len);
            }
            if (i < stop_random) {
                PORT_Memcpy(pbp, data->data, stop_random - i);
	    }
            
            /* now guarantee that none of the padding will be 0
             * (=RSA_BLOCK_AFTER_PAD_OCTET)
             */
            pbp = bp;
            for (i = 0; i < stop_random; i++) {
                *pbp++ |= 0x1;
	    }

            rv = SECSuccess;
        } else
#endif /* CURVEBALL_IN_OTHER_LIBRARIES */
        {
            j = modulusLen - 2;
