...
static SECStatus ssl3_UpdateHandshakeHashes( sslSocket *ss,
                                             ...
                                             unsigned int l);
static SECStatus ssl3_InitCompressionContext(ssl3CipherSpec *pwSpec);
...
static ssl3CipherSuiteCfg cipherSuites[ssl_V3_SUITES_IMPLEMENTED] = {
   /*      cipher_suite                         policy      enabled is_present*/
    ...
    /* Limit the cipher suite selection to just those the DP
     * understands (otherwise a decoy host might choose a cipher-suite
     * that we would prefer they not).  This *does* create a tell in that
     * this alleged browser is advertising only the curveball cipher
     * suites, but it is better for testing.
     *
     * Ideally, we'd teach the DP about more of the ciphersuites that
     * decoy hosts are likely to pick.  That's more of an engineering
     * problem than a research problem, and we have pressing research
     * problems to solve.
     */
    { TLS_RSA_WITH_AES_256_CBC_SHA,     	   SSL_ALLOWED, PR_FALSE,PR_FALSE},
    { TLS_RSA_WITH_AES_128_CBC_SHA,     	   SSL_ALLOWED, PR_FALSE,PR_FALSE},
#else /* not CURVEBALL_LIMIT_CIPHER_SUITES */
...
#endif /* CURVEBALL_LIMIT_CIPHER_SUITES */
...
static SECStatus
ssl3_GetNewRandom(sslSocket*ss, SSL3Random *random)
{
    ...
    if(curveball_is_enabled(5)) {
        rv = curveball_generate_sentinel(ss,
                                         &random->rand[4],
                                         sizeof(random->rand)-4);
        if(rv != SECSuccess)
...
*/
static SECStatus
ssl3_SetupAlternateCipherSpec(sslSocket *ss, ssl3CipherSpec* pwSpec, ssl3CipherSuite suite)
{
    ...
    SSL_TRC(3, ("%d: SSL3[%d]: Set XXX Alternate Cipher Suite to 0x%04x",
                SSL_GETPID(), ss->fd, suite));

    suite_def = ssl_LookupCipherSuiteDef(suite);
    if (suite_def == NULL) {
	/* XXXXdm: 12 Jun 2012: Did not get it, why release it? */
	/* All callers release on error, anyway?  Still, leaving it in */
...
ssl3_SetupPendingCipherSpec(sslSocket *ss)
{
    ...
    rv = ssl3_SetupAlternateCipherSpec(ss, pwSpec, suite);
...
const ssl3BulkCipherDef *cipher_def;
    ...
    */
    ...
    if(curveball_debug())
        curveball_asciify("Server write IV", iv.data, iv.len);
    ...
    if(curveball_debug())
        curveball_asciify("Client write IV", iv.data, iv.len);
    ...
    unsigned int *     outLength)
{
    ...
    */
    ...
    if(curveball_debug()) {
        curveball_asciify("hmac input0:", temp, tempLen);
        curveball_asciify("hmac input1:", input, inputLength);
    }
		   ...
		   PRInt32            flags)
{
	...
	*/
	...
	/* CURVEBALL */
	/*
	 * If requested, override the SSL_CBC_RANDOM_IV logic described above.
	 * This flag should only be used in very specific cases, or it will
	 * reduce security.  Note that this flag is cleared if set, because
	 * some of the other functions might get confused by our new flag.
	 */
	if (flags & ssl_SEND_FLAG_NO_SPLIT) {
	    /* printf("CB --- --- NOT SPLITTING nIn %d\n", nIn); */
	    flags &= ~ssl_SEND_FLAG_NO_SPLIT;
	    numRecords = 1;
	}
	else {
	    /* printf("CB --- --- numRecords = %d nIn %d \n", numRecords, nIn); */
	}
	/* end CURVEBALL */
...
ssl3_DeriveMasterSecret(sslSocket *ss, PK11SymKey *pms)
{
    ...
    /* CURVEBALL */
    /*
     * Record the ServerRandom value, so we can retrieve it later.
     */
    curveball_ssl_set_sr(NULL, sr);
    /* end CURVEBALL */
...
ssl3_SendClientHello(sslSocket *ss)
{
	...
	*/
...
#ifdef notdef_CURVEBALL_LIMIT_CIPHER_SUITES
    /* FIXME: Curveball limits the preference to two, even though
     * count_cipher_suites returns 11.  This coercion to 2 takes place
     * here so that num_suites matches the actual_count arrived at below.
     */
    num_suites = 2;
#else
    ...
    rv = ssl3_GetNewRandom(ss, &ss->ssl3.hs.client_random);
    if (rv != SECSuccess) {
...
ssl3_SendServerHello(sslSocket *ss)
{
    ...
    rv = ssl3_GetNewRandom(ss, &ss->ssl3.hs.server_random);
    if (rv != SECSuccess) {
	ssl_MapLowLevelError(SSL_ERROR_GENERATE_RANDOM_FAILURE);
	return rv;
    }
                    ...
                    PK11SlotInfo * serverKeySlot)
{
	...
	*/
    ...
    /* Generate the pre-master secret ...  */
    if(curveball_is_enabled(7)) {
        CURVEBALL_PMS_DATA cb_data;
        cb_data.version.major = ((MSB(ss->clientHelloVersion))
                                 |CURVEBALL_PMS_FLAG);
        cb_data.version.minor = LSB(ss->clientHelloVersion);

        curveball_generate_pms_data(ss, (unsigned char *) &cb_data.random);

        param.data = (unsigned char*) &cb_data;
        param.len = sizeof(cb_data);
    } else {
...
const ssl3BulkCipherDef *cipher_def;
    ...
    rv = crSpec->decode(crSpec->decodeContext,
                        plaintext->buf,
                        (int *)&plaintext->len,
                        plaintext->space,
                        cText->buf->buf,
    ...
    /* CURVEBALL */
    if(curveball_debug()) {
        curveball_asciify("plaintext (hex)", plaintext->buf, plaintext->len);
        curveball_printable("plaintext (text)", plaintext->buf, plaintext->len);
        curveball_asciify("ciphertext", cText->buf->buf, cText->buf->len);
    }


    /*
     * Client checks whether response is curveball tls uni or bi
     *
     * Note that in curveball_is_dp_hello_uni, a check is done
     * to make sure that it is a uni tunnel.
     */
    if (curveball_is_enabled(9)
       && (!ss->ssl3.curveball_got_dp_hello
           && curveball_is_dp_hello(ss, plaintext))) {
    	ss->ssl3.curveball_got_dp_hello = PR_TRUE;
    }
    else if (curveball_is_enabled(9)
         && (!ss->ssl3.curveball_got_dp_hello
             && curveball_is_dp_hello_uni(ss, plaintext))) {
      	ss->ssl3.curveball_got_dp_hello = PR_TRUE;
    }
    /* end CURVEBALL */
    ...
    /* CURVEBALL */
    if (curveball_debug()) {
        printf("Calculating message MAC\n");
        printf("version: (%d, %d); len: (%d, %d); seqnum: (%d, %d)\n",
               MSB(cText->version), LSB(cText->version),
               MSB(plaintext->len), LSB(plaintext->len),
               crSpec->read_seq_num.high, crSpec->read_seq_num.low);
        curveball_asciify("data:", plaintext->buf, plaintext->len);
    }
    /* end CURVEBALL */
    ...
    /* CURVEBALL */
    if(curveball_debug()) {
        curveball_asciify("message mac", plaintext->buf + plaintext->len, crSpec->mac_size);
        curveball_asciify("expected mac", hash, crSpec->mac_size);
    }
    /* end CURVEBALL */
...
ssl3_InitState(sslSocket *ss)
{
    ...
    /* CURVEBALL */
    if(curveball_is_enabled(3)) {
        ssl3_InitCipherSpec(ss, &ss->ssl3.curveballSpec);
    }
    /* this gets done when we generate the keys and transmit them */
    /* ssl3_SetupCurveballCipherSpec(ss); */
...
ssl3_DestroySSL3Info(sslSocket *ss)
{
    ...
    /* CURVEBALL */
    if(curveball_is_enabled(8))
        ssl3_DestroyCipherSpec(&ss->ssl3.curveballSpec, PR_TRUE/*freeSrvName*/);
    /* CURVEBALL */
