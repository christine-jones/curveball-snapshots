#define DEBUG 1
/* We only want to offer those cipher suites that we understand, otherwise
 * a decoy host might choose a non-curveballable ciphersuite.
 *
 * See ssl3con.c
 *
 * In addition, if NSS_ENABLE_ECC is defined, but we also limit things to
 * the Curveball cipher suites, there's an assert in ssl3ecc.c that is
 * ifdefed out.
 */
...
/* CURVEBALL */
/*
 * Sometimes we want to be able to disable the TLS 1.0 CBC workaround
 * (of splitting all records of length N into two records of length 1
 * and N-1), because we're already doing this splitting at a higher level
 * and we don't want to turn a record of length N into three records of
 * length 1, 1, and N-2.
 */
#define ssl_SEND_FLAG_NO_SPLIT		0x01000000
/* end CURVEBALL */
...
#define ssl_V3_SUITES_IMPLEMENTED 2
#else
...
struct ssl3StateStr {
    ...
    */
    ...
    ssl3CipherSpec       curveballSpec;
    PRBool               curveball_got_dp_hello;
    PRBool               curveball_got_dp_hello_uni;
...
void      curveball_thread_init(sslSocket*);
