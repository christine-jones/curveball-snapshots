...
ssl_SecureSend(sslSocket *ss, const unsigned char *buf, int len, int flags)
{
    ...
    /* CURVEBALL */
    /* Check whether the caller wanted to set any extra Curveball flags.
     * We clear the flags after each use, so we can set the new value
     * and retrieve the old value in one operation
     */
    flags = curveball_ssl_set_flags(0);
    /* end CURVEBALL */
